                    ...
                    <form method="POST" action="{{ route('admin.groups.update', $group) }}">
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="$group->name" required autofocus />
                                <x-input-error class="mt-2" :messages="$errors->get('name')" />
                            </div>

                            <div>
                                <x-input-label for="description" value="Description" />
                                <textarea id="description" name="description" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" rows="3">{{ $group->description }}</textarea>
                                <x-input-error class="mt-2" :messages="$errors->get('description')" />
                            </div>

                            <div>
                                <x-input-label value="Clients in this Group" />
